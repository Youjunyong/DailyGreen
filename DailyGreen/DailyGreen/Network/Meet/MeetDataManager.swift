...
class MeetDataManager {
    
    func getMeetData(delegate: MeetingViewController, communityIdx: Int, page: Int) {
        
        let headers: HTTPHeaders = ["X-ACCESS-TOKEN": Constant.TEST_TOKEN] // 테스트 토큰
        AF.request("\(Constant.BASE_URL)/app/communities/\(communityIdx)/clubs?page=1",
                   method: .get,
                   parameters: nil,
